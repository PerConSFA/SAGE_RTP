...
Const_promoter_stats_indiv_org <- function(
...
## has excluded promoters that are not classified as                          ##
...
## condition sensitivity status. If never_outlier. If condition 
...
dotplot_file_name2 <- 
  paste("./",
        ...
        "_graphics/",
        Output_file_label,
        "_Supplemental scatter_barcode.svg",
        sep="")
...
dotplot_file_name3 <- 
  paste("./",
        ...
        "_graphics/",
        Output_file_label,
        "_Supplemental scatter_condition.svg",
        sep="")
...
dotplot_file_name4 <- 
  paste("./",
        ...
        "_graphics/",
        Output_file_label,
        "_Supplemental scatter_barcode_bc_outlier.svg",
        sep="")
...
dotplot_file_name5 <- 
  paste("./",
        ...
        "_graphics/",
        Output_file_label,
        "_Supplemental scatter_condition_bc_outlier.svg",
        sep="")

ggsave(file=dotplot_file_name2, plot=all_bc_by_barcode_points, 
       width=80, height=7, limitsize = FALSE)

ggsave(file=dotplot_file_name3, plot=all_bc_by_condition_points, 
       width=80, height=7, limitsize = FALSE)

ggsave(file=dotplot_file_name4, plot=all_bc_by_barcode_points_outliers, 
       width=80, height=7, limitsize = FALSE)

ggsave(file=dotplot_file_name5, plot=all_bc_by_condition_points_outliers, 
       width=80, height=7, limitsize = FALSE)
...
Promoter_overview <- ggplot(consistent_promoters,
  ...
  # generates error bars with the standard deviation (sd) in the 
  # promoter strength as the error bars
  geom_errorbar(
...
## Many of these are removed due to lack of RNA reads, as we did not include 
## samples with 0 RNA reads for analysis, as imputing an arbitrary non-zero 
## value for analysis added a lot of noisy values for weak promoters
