...
if [ -z "${search_dir}" ]; then # if no args, output usgae and exit
	echo "Usage:"
	echo "./01_DRAFTS_process_raw.sh [search_dir] [out_dir (optional)]"
	exit 1
fi
...
while read -r fn; do
	bash ~/SBD/DRAFTS/bbmap/bbmerge.sh in1=$out_dir/"$fn"_R2.fastq in2=$out_dir/"$fn"_R1.fastq out=$out_dir/"$fn"-merged.fastq # outu1=$out_dir/unmerged-"$fn"-R2.fastq outu2=$out_dir/unmerged-"$fn"-R1.fastq
